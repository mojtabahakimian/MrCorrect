...
namespace AUTO_BAZ.Functions
{
    ...
    ///
    /// اختلاف‌های واقعی که پیدا شد:
    ///   ۱. سربرگ مبنا: GENSANADFROOSH تخفیف/ارزش‌افزوده را از سطر «فاکتور»
    ///      (HEAD_LST با TAG = 13) می‌خواند، پروسیجر از سطر «حواله» (TAG = 2).
    ///   ۲. گِردکردن: C# پیش‌فرض «به نزدیک‌ترین زوج» گِرد می‌کند، T-SQL
    ///      «نیم به بالا»؛ روی نرخ‌هایی مثل ۲٫۵٪ نتیجه یک ریال فرق می‌کرد و
    ///      همان یک ریال سطر را برای همیشه مغایر نگه می‌داشت.
    ///   ۳. الگوی تکراری/بی‌نرخ: GENSANADFROOSH سطر تکراری در
    ///      VISITORS_PORSANT_KALA را «بدون نرخ» حساب می‌کند، پروسیجر آن را
    ///      دوباره جمع می‌زد.
    ///   ۴. فاکتورهایی که GENSANADFROOSH اصلاً پردازش نمی‌کند (بدون سطر
    ///      TAG = 13، جمع فاکتور صفر یا تاریخ نامعتبر) در فهرست می‌آمدند در حالی
    ///      که هیچ صدور سندی هرگز مبلغشان را عوض نمی‌کرد.
    ///
...
FROM dbo.VISITOR_DTL AS vd
    ...
    LEFT JOIN dbo.CUST_HESAB AS chv ON chv.hes = vd.CUST_NO
WHERE vd.TAG = 2
      AND ISNULL(vd.STAT, 0) = 0
      AND b.JAMF > 0
