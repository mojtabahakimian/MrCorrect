...
namespace AUTO_BAZ.Functions
{
    ...
    ///
    /// پروسیجر dbo.CalculateVisitorPorsant (که فرم فاکتور فروش هنگام ذخیره صدا می‌زند و
    /// در مخزن ScriptSqly است) هم با همین قاعده هم‌راستا شده است. سه اختلافی که قبلاً
    /// اینجا به‌عنوان «باقی‌مانده» فهرست شده بود، در همان مخزن رفع شده‌اند:
    ///   • گِردکردن حالا ردیف‌به‌ردیف است — SUM(ROUND(...)) نه ROUND(SUM(...)).
    ///   • الگو اول از سطرِ خودِ فاکتور (VISITOR_DTL.PORID) خوانده می‌شود و فقط اگر
    ///     نبود سراغ الگوی پیش‌فرضِ ویزیتور (SALA_DTL.PORID) می‌رود؛ انتخابِ کاربر
    ///     دیگر بازنویسی نمی‌شود.
    ///   • LEFT JOIN مستقیم به VISITORS_PORSANT_KALA با یک زیرپرسشِ گروه‌بندی‌شده
    ///     (GROUP BY CODE HAVING COUNT(PORSANT) = COUNT(*) AND MIN = MAX) عوض شده،
    ///     یعنی دقیقاً همان قاعده‌ی اینجا: تکراریِ هم‌نرخ یک‌بار، تکراریِ ناهم‌نرخ هیچ.
    /// همچنین مبنای تخفیف/ارزش‌افزوده در آن رویه از سربرگ TAG = 13 خوانده می‌شود (با
    /// بازگشت به سربرگ خودِ تگ اگر سطر ۱۳ نباشد) و گزینه‌ی ۶۲ سازمان را هم مثل
    /// PorsantBase اینجا لحاظ می‌کند.
    ///
    /// دو اختلافِ باقی‌مانده که عمدی و بی‌اثرند:
    ///   • آن رویه ردیف جایزه (INVO_LST.JAY &lt;&gt; 0) را از جمعِ الگو کنار می‌گذارد و
    ///     اینجا چنین شرطی نیست — بی‌اثر، به دلیلی که پایین‌تر توضیح داده شده.
    ///   • برای سطر «مبلغ ثابت» (STAT = 1) آن رویه هیچ چیز جز LOG را دست نمی‌زند،
    ///     ولی GENSANADFROOSH درصدِ نمایشیِ سطر را از روی مبلغِ ثابت بازسازی می‌کند.
    ///     مبلغ در هر دو مسیر دست‌نخورده می‌ماند، پس حلقه‌ی مغایرت نمی‌سازد و پنجره‌ی
    ///     کنترل هم سطرهای STAT &lt;&gt; 0 را اصلاً فهرست نمی‌کند.
    ///
    /// ⚠️ اگر قاعده اینجا عوض شد، همان تغییر باید در dbo.CalculateVisitorPorsant
    /// (ScriptSqly.Core/ScriptSqly.Main.cs) هم اعمال شود، وگرنه همان حلقه‌ی
    /// «اصلاح ← صدور سند ← دوباره مغایر» برمی‌گردد.
    ///
