            ...
            using (var db = new SqlConnection(CL_CCNNMANAGER.CONNECTION_STR))
            {
                ...
                //بررسی مالکیت فاکتور و محاسبه پورسانت به صورت هوشمند
                {
                    string sqlscript = @"SET ANSI_NULLS ON;
									...
									IF OBJECT_ID('dbo.CalculateVisitorPorsant', 'P') IS NOT NULL
									BEGIN
									...
									BEGIN
									        ...
									        BEGIN
									            IF @TAG IN (2, 13)
													BEGIN
														...
														DECLARE @TaskID_Sale INT, @TaskOwner_Sale NVARCHAR(50);
														SELECT TOP 1 @TaskID_Sale = IDNUM FROM dbo.EVENTS WHERE num = @HovalehNumber AND tg IN (2, 13);
														
														IF @TaskID_Sale IS NOT NULL
														BEGIN
															SELECT @TaskOwner_Sale = USERNAME FROM dbo.TASKS WHERE IDNUM = @TaskID_Sale;
															SELECT @VisitorID = HES FROM dbo.SALA_DTL WHERE IDD = dbo.GETUSERCOD(@TaskOwner_Sale);
														END
													END
													ELSE
													BEGIN
														...
														SELECT TOP 1 @TaskID_General = IDNUM FROM dbo.EVENTS WHERE num = @NUMBER AND tg = @TAG;
											
														IF @TaskID_General IS NOT NULL
														BEGIN
															SELECT @TaskOwner_General = USERNAME FROM dbo.TASKS WHERE IDNUM = @TaskID_General;
															SELECT @VisitorID = HES FROM dbo.SALA_DTL WHERE IDD = dbo.GETUSERCOD(@TaskOwner_General);
														END
													END
									        END;
									    END;
									    ...
									    --    SET @IdentificationMethod = N'ویزیتور به صورت دستی ارائه شد';
									    ...
									    IF @PORID IS NULL
									    BEGIN
									        PRINT N'خطا: الگوی پورسانت (PORID) برای حساب ویزیتور یافت نشد: ' + @VisitorID;
									        RETURN;
									    END;
									    ...
									    IF EXISTS (SELECT 1 FROM dbo.VISITOR_DTL WHERE NUMBER = @NUMBER AND TAG = @TAG AND CUST_NO = @VisitorID AND STAT = 1)
									    BEGIN
									        -- اگر وجود داشت، از به‌روزرسانی صرف نظر کرده و هشدار می‌دهیم
									        PRINT N'هشدار: به‌روزرسانی انجام نشد. مبلغ پورسانت برای این فاکتور به صورت ثابت (STAT=1) ثبت شده و قابل تغییر خودکار نیست.';
									    END
									    ELSE
									    BEGIN
									        ...
									        SET
									            PURSANT = ROUND(@TotalPorsant, 0),
									            DARSAD = @Darsad,
									            PORID = @PORID,
									            TOZIH = @IdentificationMethod
									        WHERE
									            NUMBER = @NUMBER AND TAG = @TAG AND CUST_NO = @VisitorID;
									
									        IF @@ROWCOUNT = 0
									        BEGIN
									            INSERT INTO dbo.VISITOR_DTL (NUMBER, TAG, CUST_NO, DARSAD, PURSANT, PORID, STAT, TOZIH)
									            VALUES (@NUMBER, @TAG, @VisitorID, @Darsad, ROUND(@TotalPorsant, 0), @PORID, 0, @IdentificationMethod);
									        END;
									
									        -- فقط در صورتی که عملیات انجام شده باشد، پیام موفقیت را نمایش می‌دهیم
									        PRINT N'محاسبه پورسانت با موفقیت برای شماره سند: ' + CAST(CAST(@NUMBER AS BIGINT) AS VARCHAR) + N' و ویزیتور: ' + @VisitorID + N' انجام شد.';
