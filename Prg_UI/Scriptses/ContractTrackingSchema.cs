...
SELECT
    ...
    UnitName = COALESCE(U.NAMES, N'واحد پایه'),
...
BEGIN
    ...
    BEGIN
        ...
        SET il.ContractID = (
            ...
            FROM dbo.CONTRACT_HED bc
            INNER JOIN dbo.CONTRACT_DTL bci ON bc.ContractID = bci.ContractID
            INNER JOIN dbo.HEAD_LST hl ON il.NUMBER = hl.NUMBER AND il.TAG = hl.TAG
            WHERE bc.CUST_NO = hl.CUST_NO AND bci.CODE = il.CODE AND bc.IsClosed = 0
...
END";
            ...
            dbms.DoExecuteSQL(dropTriggerSql);
            dbms.DoExecuteSQL(createTriggerSql);
        }
    }
}
