            ...
            try
            {
                ...
                if (sqlEx is not null)
                {
                    ...
                    if (isConnectionIssue)
                    {
                        ...
                        // اگر این خطای احراز هویت/دامنه نبود، برنامه پیش از رسیدن به اینجا چند بار
                        // به صورت خودکار برای اتصال مجدد تلاش کرده (نگاه کنید به CL_CCNNMANAGER)
                        // اما موفق نشده. این توضیح را اضافه می‌کنیم تا کاربر بداند قطعی واقعی رخ
                        // داده، نه یک خطای لحظه‌ای. برای خطاهای احراز هویت/دامنه تلاش مجدد بی‌فایده
                        // است، پس این توضیح اضافه نمی‌شود.
                        if (!CL_CCNNMANAGER.IsNonRetriableAuthenticationError(sqlEx))
                        {
                            userMessage = "ارتباط با سرور پایگاه داده قطع شده است. برنامه چند بار به صورت خودکار تلاش کرد دوباره متصل شود اما موفق نشد.\n" + userMessage;
                        }
                    }
                    ...
                    else if (ctrlPressed) // Handle Ctrl+Key combinations
                    {
                        switch (pressedKey)
                        {
                            ...
                            case Key.F9:
                                Application.Current.Dispatcher.Invoke(() =>
                                {
                                    CL_MenuManager.OpenWinMenu(CL_MenuManager.WinNameType.BEDEHKARAN_BESTANKARAN_LIMITED, default);
                                });
                                break;
                            case Key.D7:
                                Application.Current.Dispatcher.Invoke(() =>
                    ...
                    else //if (noModifiers)
                    {
                        switch (pressedKey)
                        {
                            ...
                            case Key.F9:
                                Application.Current.Dispatcher.Invoke(() =>
                                {
                                    CL_MenuManager.OpenWinMenu(CL_MenuManager.WinNameType.BEDEHKARAN_BESTANKARAN, default);
                                });
                                break;
                            case Key.F10:
                                Application.Current.Dispatcher.Invoke(() =>
