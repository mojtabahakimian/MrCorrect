...
namespace Wins.WinMenus.KHARID_FORUSH
{
    public partial class HEAD_LST_FROOSH_BACK2 : Window, ISearchableWindow
    {
        ...
        /// 4
        /// </summary>
        public byte FTAG { get; } = 4; //هدر برگشت فروش

        /// <summary>
        /// 2
        /// </summary>
        public byte FAC2TAG { get; } = 2; //حواله فروش
        ...
        public bool AllowDeletions
        {
            ...
            set
            {
                ...
                IntPtr handle = new WindowInteropHelper(this).Handle; // Get the window handle

                if (handle != IntPtr.Zero) // Only proceed if the handle is valid
                {
        ...
        public bool AllowEdits
        {
            ...
            set
            {
                ...
                BUTTON_SAVE_Sayer.IsEnabled = ican;
                DELETE_SAYER.IsEnabled = ican;
                //********
            }
        }
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                JAY_COLUMN.Visibility = Visibility.Visible; //this.JAY_COLUMN .ColumnHidden = false;
                JAYO_COLUMN.Visibility = Visibility.Visible; //this.JAY_COLUMN .ColumnHidden = false;
            }
            else
            {
        ...
        {
            //if (new WindowInteropHelper(this)?.Handle == null)
            //{
            //    this.Close();
            //    return;
            //}

            CL_HESABDARI.SETSECURITY(this.GetType().Name, "FACTBFR", new WindowInteropHelper(this).Handle, this.GetType().Name);
        ...
        {
            //شماره حواله انبار 
            NUMBER1.ItemsSource = dbms.DoGetDataSQL<QRE_LST_BARGASHT>($"SELECT HEAD_LST.NUMBER FROM dbo.HEAD_LST WHERE (((HEAD_LST.TAG)={FAC2TAG})) ORDER BY HEAD_LST.NUMBER").ToList();
        }
        ...
        public void INVO_LST_SUB_ReGetData()
        {
            if (NUMBER1.SelectedValue != null)
            {
                var QRE_LST = dbms.DoGetDataSQL<INVO_LST_FACTOR22>($@"SELECT dbo.INVO_LST.NUMBER, dbo.INVO_LST.TAG, dbo.INVO_LST.ANBAR, dbo.INVO_LST.RADIF, dbo.INVO_LST.CODE, dbo.STUF_DEF.NAME AS NAME_CODE, dbo.INVO_LST.MEGH, dbo.INVO_LST.MEGHk, 
	                 ...
	                 WHERE	(dbo.INVO_LST.TAG = {FAC2TAG}) AND (dbo.INVO_LST.NUMBER={NUMBER1.SelectedValue}) ").ToList(); //-- NUMBER1
        ...
        private void INVO_LST_SUB_RowEditEnding(object sender, DataGridRowEditEndingEventArgs e)
        {
            ...
            #region INVO_LST_BACK_SUB_Exit
            if (Baseknow.TKHF > 1)
            {
                var jamtResult = dbms.DoGetDataSQL<double?>(
                    $"SELECT SUM((N_MOIN / MEGHk) * MEGH_MAR) AS JAMT FROM dbo.INVO_LST WHERE NUMBER = @Number AND TAG = {FAC2TAG}",
                    new { Number = NUMBER1.SelectedValue }).FirstOrDefault();

                if (jamtResult.HasValue && Math.Round((double)jamtResult) != Convert.ToDouble(TAKHFIF.Text))
                {
                    TAKHFIF.Text = Math.Round((double)jamtResult).ToString();
                }
            }

            var jamimbaaResult = dbms.DoGetDataSQL<double?>(
                $"SELECT SUM(IMBAA / MEGHK * MEGH_MAR) AS JAMIMBAA FROM INVO_LST WHERE NUMBER = @Number AND TAG = {FAC2TAG}",
                new { Number = NUMBER1.SelectedValue }).FirstOrDefault();

            if (jamimbaaResult.HasValue && jamimbaaResult != Convert.ToDouble(MBAA.Text))
            {
                MBAA.Text = Math.Round((double)jamimbaaResult).ToString();
                HMBAA.Text = Baseknow.HESMBAA;
            }

            var visitorDtlData = dbms.DoGetDataSQL<VISITOR_DTL>(
                $"SELECT * FROM VISITOR_DTL WHERE TAG = {FAC2TAG} AND NUMBER = @Number",
                new { Number = NUMBER1.SelectedValue });

            if (visitorDtlData.Any()) // RST.RecordCount > 0
            {
                Msgwin msgwin3 = new Msgwin(true, "این فاکتور فروش دارای پورسانت است آیا پورسانت آن هم برگشت داده شود؟"); msgwin3.ShowDialog();
                if (msgwin3.DialogResult == true)
                {
                    //point1
                    var RST2 = dbms.DoGetDataSQL<VISITOR_DTL>($"SELECT * FROM VISITOR_DTL WHERE TAG = {FTAG} AND NUMBER = " + NUMBER.Text).FirstOrDefault();
                    if (RST2 != null)
                    {
                        dbms.DoExecuteSQL($"DELETE FROM dbo.VISITOR_DTL WHERE (NUMBER = @Number) AND (TAG = {FTAG})", new { Number = NUMBER.Text });
                    }

                    foreach (var visitorDtl in visitorDtlData)
                    {
                        long prs = 0;
                        long MBK = 0;

                        if (visitorDtl.PORID != null)
                        {
                            var invoLstData = dbms.DoGetDataSQL<dynamic>(
                                $"SELECT code, MEGH_mar * mabl + (megh_MAR/MEGH)*(imbaa - n_moin) AS mablk FROM invo_lst WHERE megh_MAR > 0 AND tag = {FAC2TAG} AND NUMBER = @Number",
                                new { Number = NUMBER1.Text });

                            foreach (var invoLst in invoLstData)
                            {
                                var porsantData = dbms.DoGetDataSQL<dynamic>(
                                    "SELECT PORSANT FROM dbo.VISITORS_PORSANT_KALA WHERE (PORID = @Porid) AND (code = @Code)",
                                    new { Porid = visitorDtl.PORID, Code = invoLst.code }).FirstOrDefault();

                                if (porsantData != null)
                                {
                                    prs += (long)Math.Round(invoLst.MABLK * porsantData.PORSANT / 100);
                                    MBK += (long)invoLst.MABLK;
                                }
                            }

                            dbms.DoExecuteSQL(@"INSERT INTO VISITOR_DTL (NUMBER, tag, cust_no, DARSAD, PORID, TOZIH, STAT, PURSANT)
                                                VALUES (@Number, @Tag, @CustNo, @Darsad, @Porid, @Tozih, @Stat, @Pursant)",
                                new
                                {
                                    Number = NUMBER.Text,
                                    Tag = FTAG,
                                    CustNo = visitorDtl.CUST_NO,
                                    //Darsad = prs / (double)MBK * 100, //     Darsad = MBK != 0 ? (float)(prs / (double)MBK * 100) : 0f,
                                    Darsad = MBK != 0 ? (double)(prs / (double)MBK * 100) : 0f,
                        ...
                        else
                        {
                            dbms.DoExecuteSQL(@"INSERT INTO VISITOR_DTL (NUMBER, tag, cust_no, DARSAD, PORID, TOZIH, STAT, PURSANT)
                                ...
                                new
                                {
                                    ...
                                    Tag = FTAG,
                                    CustNo = visitorDtl.CUST_NO,
                                    Darsad = (visitorDtl.DARSAD is null ? 0 : visitorDtl.DARSAD),
                                    Porid = visitorDtl.PORID,
                                    Tozih = visitorDtl.TOZIH,
                                    Stat = false,
            ...
            //RST
            var invoLstJoinData = dbms.DoGetDataSQL<dynamic>($@"
                                    SELECT dbo.INVO_LST.VAHED_K, dbo.invo_edam.idd, dbo.INVO_LST.CODE, dbo.invo_edam.VAHED, dbo.invo_edam.MEGHTA, 
                                    dbo.invo_edam.MEGHJAY, dbo.INVO_LST.NUMBER, dbo.INVO_LST.TAG, dbo.INVO_LST.ANBAR, dbo.INVO_LST.RADIF, 
                                    dbo.INVO_LST.MEGH, dbo.INVO_LST.MEGHk, dbo.INVO_LST.JAY, dbo.INVO_LST.JAYO, dbo.INVO_LST.id, dbo.INVO_LST.MEGH_MAR
                                    FROM dbo.INVO_LST 
                                    INNER JOIN dbo.invo_edam ON dbo.INVO_LST.id = dbo.invo_edam.idd 
                                    WHERE (dbo.INVO_LST.TAG = {FAC2TAG}) AND (dbo.INVO_LST.JAY = 0) AND dbo.INVO_LST.NUMBER = @Number", new { Number = NUMBER1.SelectedValue });
            foreach (var invoLst in invoLstJoinData)
            {
                if (invoLst.JAYO == null)
                {
                    if (invoLst.MEGHTA > 0 && invoLst.MEGHJAY > 0)
                    {
                        if (invoLst.MEGHK / invoLst.MEGHTA >= 1)
                        {
                            //Jrst.Open("SELECT MEGH , meghk , MEGH_MAR FROM dbo.INVO_LST  WHERE  TAG = 2 AND JAY = " + RST.Fields("id"), CurrentProject.Connection, adOpenKeyset, adLockOptimistic);

                            var jrstData = dbms.DoGetDataSQL<dynamic>(
                                $"SELECT MEGH, meghk, MEGH_MAR FROM dbo.INVO_LST WHERE TAG = {FAC2TAG} AND JAY = @Jay",
                                new { Jay = invoLst.id }).FirstOrDefault();

                            if (jrstData != null)
                            {
                                double nmegh = Math.Floor((invoLst.MEGHK - invoLst.MEGH_MAR) / invoLst.MEGHTA) * invoLst.MEGHJAY;
                                if (nmegh != jrstData.MEGHK)
                                {
                                    dbms.DoExecuteSQL(
                                        $"UPDATE dbo.INVO_LST SET MEGH_MAR = @MeghMar WHERE TAG = {FAC2TAG} AND JAY = @Jay",
                                        new { MeghMar = jrstData.MEGHK - nmegh, Jay = invoLst.id });
            ...
            if (!IsNull(this.NUMBER.Text) && NUMBER.Text != "0")
            {
                ...
                if (!IsNull(this.NUMBER.Text) && NUMBER.Text != "0")
                {
                    ...
                    if (!IsNull(this.NUMBER.Text))
                    {
                        ...
                        CL_HESABDARI.TR("INVO_LST", "(NUMBER = " + NUMBER.Text + $") AND (TAG = {FAC2TAG})", dt, 1);
                        CL_HESABDARI.TR("PAY_GETP", "(NUMBER = " + NUMBER.Text + $") AND (TAG = {FTAG})", dt, 1);
                        CL_HESABDARI.TR("VISITOR_DTL", "(TAG = 4 and NUMBER = " + NUMBER.Text + ")", dt, 1);
        ...
        private void BTN_DELETE_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (msgwin.DialogResult == true)
            {
                ...
                _ = AuditLogger.LogActionAsync(
                     actionType: "DELETE",
                     tableName: "فاکتور برگشت فروش - عادی",
                     recordId: NUMBER.Text,
                     oldValue: "TAG = 4",
                     newValue: null,
                     additionalInfo: $@"{this.GetType().Name} , EXE PATH : {System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}");

                //if (INVO_LST_SUB.Items.Count > 0 && INVO_LST_SUB.SelectedItem != null)
                //{
                ...
                //            var _MEGH_MAR_ = item.GetType().GetProperty("MEGH_MAR").GetValue(item);

                //            if (_id_ != null)
                //            {
                //                var items = new List<object> { item }; // Wrap the item in a list

                //                var (errorMessages, _, _, _) = IVM.CheckInventoryAndExecuteQuery<int>(new List<object> { items }
                //                    , $@"UPDATE dbo.INVO_LST SET MEGH_MAR = {_MEGH_MAR_} WHERE id = {_id_}", null, false, isBarGashti: true);
        ...
        private void PERSONEL_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (!NewRecord && PERSONEL.SelectedItem != null)
            {
                ...
                Meidnum = CL_HESABDARI.PERSONELUpdate(FTAG, Convert.ToDouble(NUMBER.Text), Convert.ToInt32(PERSONEL.SelectedValue), "'فاکتور برگشت فروش عادی  شماره: " + NUMBER.Text + " مورخ " + Strings.Format(Convert.ToInt64(DATE_N.Text.ToRawTarikh()), "####/##/##") + "  به نام: " + CL_HESABDARI.GETTAFNAME(CUST_NO.SelectedValue.ToString()) + "','" + CUST_NO.SelectedValue + "'");
        ...
        private void SGN1_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                SHARH = "'فاکتور برگشت فروش عادی شماره: " + NUMBER.Text + " مورخ " + DATE_N.Text.ToRawTarikh() + "  به نام: " + CL_HESABDARI.GETTAFNAME(this.CUST_NO.SelectedValue.ToString()) + "','" + this.CUST_NO.SelectedValue + "'";
                dbms.DoExecuteSQL("insert into tasks(PERSONEL,USERNAME,TASK,COMP_COD,STDATE,STTIME,SKID,NUM,TG,CTIM,USERCO)  values (" + Baseknow.USERCOD + ",'" + CL_HESABDARI.UCurrentUser() + "'," + SHARH + "," + CL_HESABDARI.FARSIDATE() + "," + (System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateTime.Now) * 100 + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetMinute(DateTime.Now)) + $",{FTAG}," + this.NUMBER.Text + $",{FTAG}," + " GETDATE() " + "," + Baseknow.USERCOD + ", isAutomation: true)");
                MID = CL_HESABDARI.Gettaskid(Convert.ToDouble(NUMBER.Text), FTAG);
                dbms.DoExecuteSQL("insert into events(IDNUM,USERNAME,EVENTS,STDATE,STTIME,SKID,NUM,TG)  values (" + MID + ",'" + CL_HESABDARI.UCurrentUser() + "','" + CL_HESABDARI.GETUSERNAME(Convert.ToInt32(Baseknow.USERCOD)) + Interaction.IIf((bool)SGN1.IsChecked, " : امضا شد1 ", " :امضا برداشته شد1 ") + "'," + CL_HESABDARI.FARSIDATE() + "," + (System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateTime.Now) * 100 + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetMinute(DateTime.Now)) + $",{FTAG}," + NUMBER.Text + $",{FTAG} )", isAutomation: true);
            }
        ...
        private void SGN2_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                td = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.GetCultureInfo("en-US"));
                SHARH = "'فاکتور برگشت فروش عادی  شماره: " + this.NUMBER.Text + " مورخ " + DATE_N.Text + "  به نام: " + CL_HESABDARI.GETTAFNAME(this.CUST_NO.SelectedValue.ToString()) + "','" + this.CUST_NO.SelectedValue + "'";
                dbms.DoExecuteSQL("insert into tasks(PERSONEL,USERNAME,TASK,COMP_COD,STDATE,STTIME,SKID,NUM,TG,CTIM,USERCO)  values (" + Baseknow.USERCOD + ",'" + CL_HESABDARI.UCurrentUser() + "'," + SHARH + "," + CL_HESABDARI.FARSIDATE() + "," + (System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateTime.Now) * 100 + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetMinute(DateTime.Now)) + $",{FTAG}," + this.NUMBER.Text + $",{FTAG}," + " GETDATE() " + "," + Baseknow.USERCOD + ", isAutomation: true)");
                MID = CL_HESABDARI.Gettaskid(Convert.ToDouble(NUMBER.Text), FTAG);
                dbms.DoExecuteSQL("insert into events(IDNUM,USERNAME,EVENTS,STDATE,STTIME,SKID,NUM,TG)  values (" + MID + ",'" + CL_HESABDARI.UCurrentUser() + "','" + CL_HESABDARI.GETUSERNAME(Convert.ToInt32(Baseknow.USERCOD)) + Interaction.IIf((bool)SGN2.IsChecked, ":امضا شد2 ", ":امضا برداشته شد2:") + "'," + CL_HESABDARI.FARSIDATE() + "," + (System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateTime.Now) * 100 + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetMinute(DateTime.Now)) + $",{FTAG}," + NUMBER.Text + $",{FTAG} )", isAutomation: true);
            }
        ...
        {
            //throw new Exception();
            AUTO_BAZ.Functions.CL_HESABDARI_AUTO_BAZ.gensanadbargashfroosh(Convert.ToInt64(NUMBER.Text), Convert.ToInt64(NUMBER.Text), false);
        ...
        public void PAY_GETP_SUB_SUB_ReGetData()
        {
            if (!string.IsNullOrEmpty(NUMBER.Text) && NUMBER.Text != "0") //Did Saved
            {
                ...
                var QRE_LST = dbms.DoGetDataSQL<PAY_GETP_MODEL>($@"SELECT * FROM PAY_GETP WHERE NUMBER = {NUMBER.Text} AND TAG = {FTAG} AND (N_KOL IS NULL OR N_KOL <> 911) ").ToList();
                if (QRE_LST.Count > 0)
                {
                    foreach (var item in QRE_LST)
                    {
            ...
            if (PAY_GETP_SUB.Items.Count > 0 && PAY_GETP_SUB.SelectedItem != null)
            {
                if (!(PAY_GETP_SUB.SelectedItems is null))
                {
                    ...
                    if (msgwin.DialogResult == true)
                    {
                        ...
                        CL_HESABDARI.TR("PAY_GETP", "(NUMBER = " + this.NUMBER.Text + $") AND (TAG = {FTAG})", dt, 1); //1

                        _ = AuditLogger.LogActionAsync(
                                actionType: "DELETE",
                                tableName: "فاکتور برگشت فروش - عادی => چک های پرداختی پشت فاکتور",
                                recordId: NUMBER.Text,
                                oldValue: "TAG = 4",
                                newValue: null,
                                additionalInfo: $@"{this.GetType().Name} , EXE PATH : {System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}");
        ...
        public void VISITOR_DTL_SUB_ReGetData()
        {
            ...
            {
                SAYER_VISITOR_DATA?.Clear();
                var QRE_LST = dbms.DoGetDataSQL<VISITOR_DTL>($@"SELECT * FROM VISITOR_DTL WHERE NUMBER = {NUMBER.Text} AND TAG = {FTAG}").ToList();
                if (QRE_LST.Count > 0)
                {
                    foreach (var item in QRE_LST)
                    {
        ...
        private double _sum_of_pursant = 0;
        public double SUM_OF_PURSANT
        {
            get
            {
                _sum_of_pursant = (double)SAYER_VISITOR_DATA.Sum(r => r.PURSANT);
                if (_sum_of_pursant == 0) _sum_of_pursant = 0;
                return _sum_of_pursant;
            }
        ...
        private void VISITOR_DTL_SUB_PreviewMouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            if (grid.SelectedItem == CollectionView.NewItemPlaceholder) return;
            if (grid.SelectedItem is not VISITOR_DTL currentData) return;

            try
            {
                if (grid is null || grid.CurrentCell.Column is null || !grid.CurrentCell.IsValid) return;
                ...
                if (grid.CurrentCell.Column != null)
                {
                    ...
                    if (grid.CurrentCell.Column.SortMemberPath == "PORID")
                    {
                        ...
                        if (rst.Count > 0 & !IsNull(rst.FirstOrDefault().PORID))
                        {
                            ...
                            {
                                var ROWS = dbms.DoGetDataSQL<QRE_VISIT1>($"SELECT CODE ,MEGH_MAR * MABL + (MEGH_MAR/MEGH)*(IMBAA - N_MOIN) AS MABLK FROM INVO_LST WHERE TAG = {FAC2TAG} AND NUMBER = " + this.NUMBER.Text).ToList();
        ...
        private void VISITOR_DTL_SUB_CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
        {
            ...
            double _JF_ = Convert.ToDouble(JF.Text);
            double _TAKHFIF_ = Convert.ToDouble(TAKHFIF.Text);
            ...
            if (e.Column.SortMemberPath == "CUST_NO_NAME") //CUST_NO_NAME == CUST_NO 112-1-1 محمدی دهقان تستی
            {
                ...
                if (_VISITOR_DTL_WAS_ROW_ITEM.CUST_NO_NAME != VISITOR_DTL_SUB_ENTERED_VALUE) // نام مشتری جدید وارد شده
                {
                    ...
                    {
                        ComboSearch CMBSearch = new ComboSearch("POSHTE_FACTOR_BACK", I_AM_BARGASHT_NORMAL);//Search Plusy Form Specialy for Customers
                        CMBSearch.ShowDialog();
            ...
            if (e.Column.SortMemberPath == "DARSAD")
            {
                ...
                //CURRENT_ROW_VISITOR.PURSANT = Math.Round((Convert.ToDouble(JF.Text) - Convert.ToDouble(TAKHFIF.Text)) * this.DARSAD / 100);
            ...
            if (e.Column.SortMemberPath == "PORID")
            {
                if (!string.IsNullOrEmpty(VISITOR_DTL_SUB_ENTERED_VALUE))
                {
                    ...
                    var RST = dbms.DoGetDataSQL<QVIS2>($"SELECT CODE ,MEGH_MAR * MABL + (MEGH_MAR/MEGH)*(IMBAA - N_MOIN) AS MABLK FROM INVO_LST WHERE TAG = {FAC2TAG} AND NUMBER = " + NUMBER.Text).ToList();

                    foreach (var row in RST) // while (!RST.EOF)
                    {
                        var RST2 = dbms.DoGetDataSQL<double?>("SELECT  PORSANT FROM dbo.VISITORS_PORSANT_KALA WHERE  (PORID = " + VISITOR_DTL_SUB_ENTERED_VALUE + ") AND (CODE = '" + row.CODE + "')").ToList();
                        if (RST2.Count == 1)
                        {
                            prs = (long)(prs + Math.Round((double)(row.MABLK * RST2.FirstOrDefault(/*PORSANT*/) / 100)));
                            MBK = (long)(MBK + row.MABLK);
                        }
                        else
                        {
                            new Msgwin(false, "تذكر مهم :اين كالا فاقد الگو براي اين ويزيتور است و پورسانت محاسبه نشد.درصورت لزوم براي آن تعريف كنيد و همينجا مجددا الگو را انتخاب كنيد  : " + CL_HESABDARI.GETKALANAME(Convert.ToDouble(row.CODE))).ShowDialog();
                        }
                    }

                    CURRENT_ROW_VISITOR.PURSANT = prs;
                    if (MBK > 0L & prs > 0L)
                    {
        ...
        private void VISITOR_DTL_SUB_RowEditEnding(object sender, DataGridRowEditEndingEventArgs e)
        {
            ...
            //{
            //    ErrosMessages.Add(new MsgModel { MessageText_U = "الگوي پرداخت پورسانت خالی است!" });
            //}
            ...
            if (VISITOR_DTL_SUB.Items.Count > 0 && VISITOR_DTL_SUB.SelectedItem != null)
            {
                if (!(VISITOR_DTL_SUB.SelectedItems is null))
                {
                    ...
                    if (msgwin.DialogResult == true)
                    {
                        ...
                        _ = AuditLogger.LogActionAsync(
                                actionType: "DELETE",
                                tableName: "فاکتور برگشت فروش - عادی  => پورسانت ویزیتور ها",
                                recordId: NUMBER.Text,
                                oldValue: "TAG = 4",
                                newValue: null,
                                additionalInfo: $@"{this.GetType().Name} , EXE PATH : {System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}");
                        ...
                        for (int i = 0; i < VISITOR_DTL_SUB.SelectedItems.Count; i++)
                        {
                            ...
                            if (!(item.ToStringNullSafe() is "{NewItemPlaceholder}"))
                            {
                                ...
                                dbms.DoExecuteSQL($"DELETE FROM dbo.VISITOR_DTL WHERE NUMBER = {NUMBER.Text} AND TAG = {FTAG} AND " +
        ...
        {
            if (VISITOR_DTL_SUB.IsEnabled && !VISITOR_DTL_SUB.IsReadOnly)
            {
        ...
        private void Command100_Click(object sender, RoutedEventArgs e)
        {
            if (SUM_OF_MEGH_MAR <= 0)
            {
                new Msgwin(false, "مقدار مرجوعی وجود ندارد !").ShowDialog();
                return;
            }
            ...
            var pathreport = Assembly.GetEntryAssembly().GetManifestResourceStream("Prg_UI.Rpts.Factors.INVOIC_FROOSH_BACK2.mrt");
            ...
            var rst = dbms.DoGetDataSQL<CheckData>($@"
                                                      ...
                                                      WHERE PAY_GETP.NUMBER = {NUMBER.Text} AND PAY_GETP.TAG = {FTAG} AND (N_KOL IS NULL OR N_KOL <> 911)").ToList();
            ...
            var jst = dbms.DoGetDataSQL<double?>($@"
                                                    ...
                                                    FROM INVO_LST 
                                                    WHERE INVO_LST.NUMBER = {THEHEADERNUM} AND INVO_LST.TAG = {FAC2TAG}").FirstOrDefault();
            ...
            //var rst03 = dbms.DoGetDataSQL<double?>("SELECT  SUM(dbo.STUF_DEF.VAZN * dbo.INVO_LST.MEGHk) AS Weight FROM dbo.INVO_LST INNER JOIN dbo.STUF_DEF ON dbo.INVO_LST.CODE = dbo.STUF_DEF.CODE WHERE (dbo.INVO_LST.TAG = " + FAC2TAG /*TAG = 9 */ + ") AND (dbo.INVO_LST.NUMBER = " + THEHEADERNUM + ")").ToList();
            //if (rst03.Count > 0)
            //{
        ...
        private void Command108_Click(object sender, RoutedEventArgs e)
        {
            ...
            var jst = dbms.DoGetDataSQL<double?>($@"
                                                    SELECT SUM(INVO_LST.MABL_K) AS SumOfMABL_K 
                                                    FROM INVO_LST 
                                                    WHERE INVO_LST.NUMBER = {NUMBER.Text} AND INVO_LST.TAG = {FTAG}").FirstOrDefault();
            ...
            //JF جمع مبلغ
            if (report.GetComponentByName("JF") is StiText stiText1) stiText1.Text = JF.Text;
        ...
        private void NUMBER1_PreviewLostKeyboardFocus(object sender, KeyboardFocusChangedEventArgs e)
        {
            ...
            SumOfMEGH_MAR = dbms.DoGetDataSQL<double?>("SELECT Sum(INVO_LST.MEGH_MAR) AS SumOfMEGH_MAR FROM INVO_LST WHERE (((INVO_LST.NUMBER)= " + NUMBER1.SelectedValue + $" ) AND ((INVO_LST.TAG)={FAC2TAG}))").FirstOrDefault();
        ...
        private void VISITOR_DTL_SUB_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
