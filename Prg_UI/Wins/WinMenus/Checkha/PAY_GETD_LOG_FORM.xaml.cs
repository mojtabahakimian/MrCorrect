...
{
    public partial class PAY_GETD_LOG_FORM : Window
    {
        public PAY_GETD_LOG_FORM(string n_seri, string bank, string date_s)
        {
            InitializeComponent();
        ...
        public ObservableCollection<HISTORY_MODEL_CHK> SFDATAGRID_DATA { get; set; } = new ObservableCollection<HISTORY_MODEL_CHK>();
        ...
        private void FILL_ALL_COMBOBOX()
        {
            ...
            //موقعیت چک
            SANDUGH_COLUMN.ItemsSource = dbms.DoGetDataSQL<TDETA_HES>($"SELECT * FROM TDETA_HES WHERE(N_KOL = 113) AND(NUMBER = 1)").ToList();
        }
        ...
        private void ReGetData()
        {
            ...
            if (N_SERI != null)
            {
                _CONDITION_ = $@"  WHERE(N_SERI ={N_SERI} AND BANK = {BANK} AND DATE_S = {DATE_S});";
            }

            SFDATAGRID_DATA?.Clear();
            var RST = dbms.DoGetDataSQL<HISTORY_MODEL_CHK>(@$"SELECT *
                                                              FROM(SELECT PAY_GETD_LOG.*, TCOD_BANKS.NAMES, PAY_GETD.SHOBEH, PAY_GETD.MABL, PAY_GETD.NAME_TAH, PAY_GETD.RADIF, PAY_GETD.CUST_NO
                                                                   FROM PAY_GETD_LOG
                                                                        INNER JOIN PAY_GETD ON PAY_GETD_LOG.N_SERI=PAY_GETD.N_SERI AND PAY_GETD_LOG.BANK=PAY_GETD.BANK AND PAY_GETD_LOG.DATE_S=PAY_GETD.DATE_S
                                                                        INNER JOIN TCOD_BANKS ON PAY_GETD.BANK=TCOD_BANKS.CODE) AS DRVD_TBL
                                                             {_CONDITION_}").ToList();
        ...
        #region _SfDataGrid_
        private readonly FilterService<HISTORY_MODEL_CHK> filterService = new FilterService<HISTORY_MODEL_CHK>();
        ...
        private void ApplyCumulativeFilter()
        {
            ...
            SFDATAGRID_SUB.View.Filter = item => filterService.ApplyFilter(item as HISTORY_MODEL_CHK);
        ...
        public void GenerateAutomaticSummary(SfDataGrid _DG_, bool _ClearAnySummaryBefore_ = false)
        {
            ...
            var dataType = typeof(HISTORY_MODEL_CHK);
            ...
            foreach (var column in _DG_.Columns.OfType<GridTextColumn>())
            {
                ...
                {
                    var propertyInfo = typeof(HISTORY_MODEL_CHK).GetProperty(column.MappingName);
        ...
        public string N_SERI { get; }
        public string BANK { get; }
        public string DATE_S { get; }
