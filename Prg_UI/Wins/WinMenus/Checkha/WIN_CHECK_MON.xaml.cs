...
{
    public partial class WIN_CHECK_MON : Window
    {
        ...
        private void ReGetData(bool CalledFromButtons = false)
        {
            ...
            string RecordSource = @$"SELECT dbo.PAY_GETD.N_SERI, dbo.PAY_GETD.BANK,
                                                           dbo.TCOD_BANKS.NAMES AS BANK_NAME,
                                                           dbo.PAY_GETD.DATE_S, dbo.PAY_GETD.DATE,
                                                           dbo.PAY_GETD.SHOBEH, dbo.PAY_GETD.MABL,
                                                           dbo.PAY_GETD.NAME_TAH, dbo.PAY_GETD.N_HESAB,
                                                           dbo.PAY_GETD.N_S, dbo.PAY_GETD.N_KOL,
                                                           dbo.PAY_GETD.N_MOIN, dbo.PAY_GETD.N_TAF,
                                                           dbo.PAY_GETD.N_KOL2, dbo.PAY_GETD.N_MOIN2,
                                                           dbo.PAY_GETD.N_TAF2, dbo.PAY_GETD.N_KOL3,
                                                           dbo.PAY_GETD.N_MOIN3, dbo.PAY_GETD.N_TAF3,
                                                           dbo.PAY_GETD.NUMBER, dbo.PAY_GETD.TAG,
                                                           dbo.PAY_GETD.ANBAR, dbo.PAY_GETD.RADIF,
                                                           dbo.PAY_GETD.CUST_NO, dbo.PAY_GETD.VAZ,
                                                           LEFT(CAST(dbo.PAY_GETD.DATE_S AS NVARCHAR(8)), 4) AS YE,
                                                           SUBSTRING(CAST(dbo.PAY_GETD.DATE_S AS NVARCHAR(8)), 5, 2) AS MO
                                                    FROM dbo.PAY_GETD
                                                        LEFT OUTER JOIN dbo.TCOD_BANKS
                                                            ON dbo.PAY_GETD.BANK = dbo.TCOD_BANKS.CODE
                                                    WHERE (LEFT(CAST(dbo.PAY_GETD.DATE_S AS NVARCHAR(8)), 4) = {_YEAR_})
                                                          AND (SUBSTRING(CAST(dbo.PAY_GETD.DATE_S AS NVARCHAR(8)), 5, 2) = {_MONTH_})
                                                          AND
                                                          (
                                                              dbo.PAY_GETD.N_KOL <> 911
                                                              OR dbo.PAY_GETD.N_KOL IS NULL
                                                          );";
            ...
            switch (GRP)
            {
                case 1:
                    {
                        //this["PAY_GETD_mon"].Form.RecordSource = "SELECT N_SERI, BANK, DATE_S, DATE, SHOBEH, MABL, NAME_TAH, N_HESAB, N_S, N_KOL, N_MOIN, N_TAF, N_KOL2, N_MOIN2, N_TAF2, N_KOL3, N_MOIN3, N_TAF3, NUMBER, TAG, ANBAR, RADIF, CUST_NO, VAZ, LEFT(CAST(DATE_S AS NVARCHAR(8)), 4) AS YE, SUBSTRING(CAST(DATE_S AS NVARCHAR(8)), 5, 2) AS MO FROM PAY_GETD WHERE     (LEFT(CAST(DATE_S AS NVARCHAR(8)), 4) = " + this.Text35 + ") AND (SUBSTRING(CAST(DATE_S AS NVARCHAR(8)), 5, 2) = " + this.Frame13 + " AND ( N_KOL <> 911 OR N_KOL IS NULL ))";

                        RecordSource = @$"SELECT dbo.PAY_GETD.N_SERI, dbo.PAY_GETD.BANK,
                                                           dbo.TCOD_BANKS.NAMES AS BANK_NAME,
                                                           dbo.PAY_GETD.DATE_S, dbo.PAY_GETD.DATE,
                                                           dbo.PAY_GETD.SHOBEH, dbo.PAY_GETD.MABL,
                                                           dbo.PAY_GETD.NAME_TAH, dbo.PAY_GETD.N_HESAB,
                ...
                case 4:
                    {
                        ...
                        RecordSource = @$"SELECT dbo.PAY_GETD.N_SERI, dbo.PAY_GETD.BANK,
                                                    ...
                                                    WHERE (LEFT(CAST(dbo.PAY_GETD.DATE_S AS NVARCHAR(8)), 4) = {_YEAR_})
                                                          ...
                                                          AND N_KOL <> 911";
                        //AND (N_KOL3 IS NULL) AND (NOT (N_KOL IS NULL) AND N_KOL <> {Baseknow.BANKHA}) AND N_KOL <> 911
                        break;
                    }
                case 5:
                    {
