    ...
    public static class CL_CONTORL_ASNAD
    {
        ...
        private static void CheckVisitorCommissions()
        {
            try
            {
                ...
                // پنجره همیشه باز می‌شود، مثل بقیه‌ی کنترل‌های همین دستور. پیش از این فقط
                // در صورت وجود مغایرت باز می‌شد و نتیجه‌اش این بود که وقتی همه‌ی فاکتورها
                // درست بودند، کاربر هیچ راهی به ستون «پوشش الگو» و دکمه‌ی «این مبلغ از کجا
                // آمده؟» نداشت — در حالی که آن ریز محاسبه دقیقاً برای فاکتورِ بدون مغایرت
                // لازم است («مبلغ درست ثبت شده، ولی چرا این‌قدر کم است؟»).
