...
{
    /// <summary>
    /// Interaction logic for SHA_LIST_WIN.xaml
    /// </summary>
    public partial class SHA_LIST_WIN : Window
    {
        ...
        public ObservableCollection<HEAD_LST> INVO_ETG_DATA { get; set; } = new ObservableCollection<HEAD_LST>();
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var MasterHead = dbms.DoGetDataSQL<HEAD_LST>(@$"SELECT HEAD_LST.* FROM HEAD_LST WHERE (TAG = 26) ORDER BY NUMBER").ToList();

            foreach (var item in MasterHead)
            {
                INVO_ETG_DATA.Add(item);
            }
        ...
        private void Window_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter && Keyboard.Modifiers == ModifierKeys.None && SYNCFUSION_DG.SelectedItem != null)
            {
                ...
                var currentRow = SYNCFUSION_DG.SelectedItem as HEAD_LST;

                if (currentRow?.NUMBER != null)
                {
                    OpenWindow(typeof(HEAD_LST_HAV_OTHER_WIN), (double)currentRow.NUMBER, "یک پنجره حواله انبار از قبل باز شده ابتدا آنرا ببندید.");
                }
