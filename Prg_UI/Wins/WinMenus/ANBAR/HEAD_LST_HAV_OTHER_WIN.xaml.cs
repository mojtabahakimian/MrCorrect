...
namespace Wins.WinMenus.ANBAR
{
    ...
    /// </summary>
    public partial class HEAD_LST_HAV_OTHER_WIN : Window, ISearchableWindow, IComboLookupProvider
    {
        ...
        private void Form_Current()
        {
            ...
            {
                var RST = dbms.DoGetDataSQL<HEAD_LST>("SELECT * FROM HEAD_LST WHERE TAG = 27 and NUMBER1 =  " + this.NUMBER.Text).ToList();
                if (RST.Count == 0)
                {
                    //this.AllowAdditions = true;
                    this.AllowDeletions = true;
        ...
        private void ESLAH_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!IsNull(this.NUMBER.Text))
            {
                ...
                else
                {
                    ...
                    AnyCrossAzadInvoiced = dbms.DoGetDataSQL<int?>($"SELECT TOP 1 NUMBER FROM dbo.HEAD_LST WHERE TAG = 27 AND NUMBER1 = {NUMBER.Text} ").Any();
